import {PackageCatalog} from '../app/utils/package-config';
import {PackageRow} from '../app/models/package-row.model';

/**
 * The seeded catalog (mirrors the service's scripts/seed-packages.ts) for
 * specs that need package definitions now that PACKAGE_CONFIG is a table.
 */
...
/** The same catalog as raw service rows, for stubbing PackageService.getPackages. */
export const TEST_CATALOG_ROWS: PackageRow[] = Object.entries(TEST_CATALOG).map(
  ([id, def]) => ({id, ...def, retired: false}),
);
